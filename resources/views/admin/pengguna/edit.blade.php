...
<div class="mb-6 flex items-center gap-4">
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Perbarui profil atau reset kredensial untuk akun <strong>{{ $pengguna->nama_lengkap }}</strong>.</p>
    </div>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 max-w-3xl">
    <form action="{{ route('admin.pengguna.update', $pengguna->id_user) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap', $pengguna->nama_lengkap) }}" required placeholder="Contoh: John Doe"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="username" name="username" value="{{ old('username', $pengguna->username) }}" required placeholder="johndoe123"
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $pengguna->email) }}" required placeholder="john@example.com"
                    ...
                    @if(Auth::id() == $pengguna->id_user)
                        <div class="bg-blue-50 border border-blue-200 p-4 rounded-xl flex gap-3">
                            ...
                            <div class="text-sm text-blue-800">
                                <p class="font-bold">Info Hak Akses</p>
                                <p class="mt-0.5 text-blue-700">Karena ini adalah akun yang sedang Anda gunakan, Anda tidak dapat mengubah Role (Hak Akses) Anda sendiri menjadi UMKM untuk mencegah kehilangan akses ke panel Admin.</p>
                            </div>
                        </div>
                    ...
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label class="relative flex items-center p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition [&:has(input:checked)]:border-brand-purple [&:has(input:checked)]:bg-purple-50/50 [&:has(input:checked)]:ring-1 [&:has(input:checked)]:ring-brand-purple group">
                                <input type="radio" name="role" value="umkm" class="w-4 h-4 text-brand-purple bg-gray-100 border-gray-300 focus:ring-brand-purple focus:ring-2" {{ old('role', $pengguna->role) == 'umkm' ? 'checked' : '' }}>
                            ...
                            <label class="relative flex items-center p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition [&:has(input:checked)]:border-brand-purple [&:has(input:checked)]:bg-purple-50/50 [&:has(input:checked)]:ring-1 [&:has(input:checked)]:ring-brand-purple group">
                                <input type="radio" name="role" value="admin" class="w-4 h-4 text-brand-purple bg-gray-100 border-gray-300 focus:ring-brand-purple focus:ring-2" {{ old('role', $pengguna->role) == 'admin' ? 'checked' : '' }}>
                                <div class="ml-3 flex-1">
        ...
        <div>
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2.5 py-1 bg-gray-100 text-gray-500 text-[10px] font-bold uppercase rounded-full tracking-wider border border-gray-200">
                    Bisa Dikosongkan
                </span>
            </div>
            
            <p class="text-[11px] text-gray-500 mb-4 bg-orange-50 border border-orange-100 p-2 rounded-lg text-orange-800">
                 Biarkan kosong jika Anda tidak ingin mereset/mengganti password dari pengguna ini.
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <!-- Konfirmasi Password -->
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 mb-1.5">Ulangi Kata Sandi Baru</label>
